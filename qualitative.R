...
viz <- c("intuitive", "detailed", "compact")
ease.intuitive.value  <- filtered.intuitive$vq1
ease.intuitive.factor <- rep(factor(viz[1]), length(filtered.intuitive$vq1))

kw.ease   <- kruskal.test(list(filtered.intuitive$vq1, filtered.detailed$vq1, filtered.compact$vq1))
kw.appeal <- kruskal.test(list(filtered.intuitive$vq3, filtered.detailed$vq3, filtered.compact$vq3))
kw.expect <- kruskal.test(list(filtered.intuitive$vq15, filtered.detailed$vq15, filtered.compact$vq15))

kw.trust       <- kruskal.test(list(filtered.intuitive$vq6, filtered.detailed$vq6, filtered.compact$vq6))
kw.confidence  <- kruskal.test(list(filtered.intuitive$vq8, filtered.detailed$vq8, filtered.compact$vq8))
kw.credibility <- kruskal.test(list(filtered.intuitive$vq7, filtered.detailed$vq7, filtered.compact$vq7))
